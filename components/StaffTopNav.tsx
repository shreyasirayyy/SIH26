...
export function StaffTopNav({ role }: { role: "counsellor" | "admin" }) {
  ...
  return <div className="min-h-screen bg-[#f7f8f3] md:flex">
    <aside className="hidden w-64 shrink-0 border-r border-border-color bg-white px-5 py-7 md:flex md:flex-col">
      ...
      <div className="mt-auto border-t border-border-color pt-4"><Button size="sm" variant="ghost" className="w-full justify-start gap-2" onClick={() => { logout(); router.push("/welcome"); }}><LogOut size={16} />Sign out</Button></div>
    </aside>
    <div className="min-w-0 flex-1"><header className="border-b border-border-color bg-white"><div className="mx-auto flex w-full items-center justify-between px-5 py-4 md:px-8"><Link href={role === "counsellor" ? "/counsellor" : "/admin"} className="font-semibold text-deep-teal md:hidden">SAATH · {role === "counsellor" ? "Counsellor" : "Admin"}</Link><p className="hidden text-sm font-semibold text-text-primary md:block">{role === "counsellor" ? "Good morning, Dr. Neha" : "Aggregated decision intelligence"}</p><Button size="sm" variant="ghost" className="md:hidden" onClick={() => { logout(); router.push("/welcome"); }}>Sign out</Button></div></header>{/* Mobile navigation keeps the same destinations reachable. */}<nav className="flex gap-2 overflow-x-auto border-b border-border-color bg-white px-5 py-2 md:hidden" aria-label="Staff navigation">{items.slice(0, 4).map(({ href, label }) => <Link key={label} href={href} className="whitespace-nowrap rounded-full bg-greenish-cream px-3 py-1.5 text-xs font-semibold text-text-secondary">{label}</Link>)}</nav></div>
  </div>;
}
