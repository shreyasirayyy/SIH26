...
The backend is an independently runnable Express/TypeScript service. HTTP composition lives in `src/app.ts`; validation is applied before route handlers; `services/case` isolates the replaceable synthetic NHAA adapter, `services/taara` isolates the AI provider boundary, and `services/notifications` supports future generic notifications. Middleware handles request IDs, docket-created JWT sessions, roles, and safe errors; the data layer exposes Supabase plus a memory mode for tests and local development.
