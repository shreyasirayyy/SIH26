...
const schema = z.object({
  ...
  AI_PROVIDER: z.string().default('fallback'), AI_API_KEY: z.string().optional(), GEMINI_API_KEY: z.string().optional(), GOOGLE_PLACES_API_KEY: z.string().optional(), GROQ_MODEL: z.string().default('openai/gpt-oss-20b'), ML_SERVICE_URL: z.string().url().optional(),
  JWT_SECRET: z.string().min(32).default('development-only-change-me-please-32-chars'), JWT_EXPIRES_IN: z.string().default('8h'), ALLOW_DEV_STAFF_TOKEN: z.coerce.boolean().default(false), OTP_TTL_SECONDS: z.coerce.number().int().positive().default(300), OTP_RESEND_COOLDOWN_SECONDS: z.coerce.number().int().nonnegative().default(60), OTP_MAX_ATTEMPTS: z.coerce.number().int().positive().default(5), OTP_PEPPER: z.string().default('development-pepper'), UPLOAD_MAX_BYTES: z.coerce.number().default(10485760), LOG_LEVEL: z.string().default('info')
});
