...
const schema = z.object({
  ...
  AI_PROVIDER: z.string().default('fallback'), AI_API_KEY: z.string().optional(), GEMINI_API_KEY: z.string().optional(), GEMINI_MODEL: z.string().default('gemini-3-flash-preview'), GOOGLE_PLACES_API_KEY: z.string().optional(), GROQ_MODEL: z.string().default('openai/gpt-oss-20b'), ML_SERVICE_URL: z.string().url().optional(), ML_API_KEY: z.string().optional(),
  JWT_SECRET: z.string().min(32).default('development-only-change-me-please-32-chars'), JWT_EXPIRES_IN: z.string().default('8h'), ALLOW_DEV_STAFF_TOKEN: z.preprocess((val) => { if (val === undefined || val === '') return (process.env.NODE_ENV || 'development') !== 'production'; return val === 'true' || val === '1' || val === true; }, z.boolean()), UPLOAD_MAX_BYTES: z.coerce.number().default(10485760), LOG_LEVEL: z.string().default('info')
});
