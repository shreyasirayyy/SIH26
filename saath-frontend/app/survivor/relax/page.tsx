...
export default function RelaxPage() {
	...
	const [language, setLanguage] = useState<RelaxLanguage>("hi");
	...
	return (
		<div className="px-5 pb-10 md:px-10 xl:px-14">
			...
			<div className="mx-auto mt-10 max-w-2xl text-center">
				...
				<p className="mt-6 text-xs font-bold uppercase tracking-[.2em] text-[#7e918b]">{content.title[language]} · 5 min</p>
				...
				<div className="surface-soft mt-10 rounded-[28px] p-8">
					...
					{!started && <p className="mt-6 text-sm text-[#6b7b75]">Take a moment to relax.</p>}
					{done && <p className="mt-6 text-sm text-[#6b7b75]">You made space for yourself.</p>}

					<div className="mt-6 flex items-center justify-center gap-3">
						<button onClick={() => setStarted(true)} className="inline-flex items-center gap-2 rounded-full bg-[#8064a2] px-6 py-3 text-sm font-bold text-white disabled:opacity-70" disabled={started}>
						...
						{started && !done && (
							<button
								...
								className="inline-flex h-11 w-11 items-center justify-center rounded-full border border-[#d8cfe8] text-[#8064a2]"
							>
