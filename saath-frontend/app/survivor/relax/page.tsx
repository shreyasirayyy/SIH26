		...
		<div className="px-5 pb-10 md:px-10 xl:px-14">
			...
			<div className="mx-auto mt-10 max-w-2xl text-center">
				...
				<div className="surface-soft mt-10 rounded-[28px] p-8">
					<div className="mx-auto flex h-28 w-28 items-center justify-center rounded-full bg-[#e8e0f3] text-3xl text-[#8064a2]">{done ? <Check /> : "05:00"}</div>
					<p className="mt-6 text-sm text-[#6b7b75]">{done ? "You made space for yourself." : started ? "Follow the gentle prompts. Nothing else is needed." : "Audio placeholder · headphones optional"}</p>
					<button onClick={() => (started ? setDone(true) : setStarted(true))} className="mt-6 inline-flex items-center gap-2 rounded-full bg-[#8064a2] px-6 py-3 text-sm font-bold text-white">
						{started ? "Complete activity" : <><Play size={15} /> Begin</>}
					</button>
				</div>
