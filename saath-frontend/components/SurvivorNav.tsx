...
export function SurvivorNav() {
  ...
  return <nav className="fixed bottom-0 left-0 right-0 z-40 border-t border-border-color/70 bg-[color:var(--surface)]/90 px-2 pb-[env(safe-area-inset-bottom)] pt-2 shadow-[0_-12px_35px_rgba(29,71,64,.08)] backdrop-blur-xl md:hidden"><ul className="mx-auto flex max-w-lg justify-between"><li className="contents">{items.map(({ href, label, icon: Icon }) => { const active = href === "/survivor" ? pathname === href : pathname.startsWith(href); return <Link key={href} href={href} className={cn("flex min-w-0 flex-1 flex-col items-center gap-1 rounded-2xl py-2 text-[10px] font-semibold", active ? "text-[color:var(--primary-teal)]" : "text-[color:var(--text-secondary)]")}><span className={cn("rounded-xl px-3 py-1", active && "bg-[color:var(--primary-teal-dark)]")}><Icon size={18} strokeWidth={active ? 2.5 : 1.8} /></span><span className="truncate">{label}</span></Link>; })}</li></ul></nav>;
}
